...
def search_command():
    list1.delete(0,END)
    for row in db.search(title=e1.get(), author=e2.get(),
                        year=e3.get(), isbn=e4.get()):
        list1.insert(END, row)

def add_command():
    if not all(x == '' for x in (e1.get(), e2.get(), e3.get(), e4.get())):
        db.add(title=e1.get(), author=e2.get(), year=e3.get(), isbn=e4.get())#u
        view_command()

def delete_command():
    try:
        db.delete(selected_listbox_row[0])
        view_command()
    except NameError:
        pass
...
def update_command():
    try:
        db.update(id=selected_listbox_row[0], title=e1.get(),
